...
func loadStaticFile(localPath string, webPath string, ignoreExceptions bool) (string, error) {
	...
	if !ignoreExceptions && isStaticFileExempt(filepath.Base(localPath)) {
		qbin.Log.Debugf("Exempted static file: %s", webPath)
	...
	if err != nil {
		qbin.Log.Errorf("Couldn't read static file '%s' for the following reason: %s", webPath, err)
		return "", err
	}

	// Log & return the file, with global variables already replaced
	qbin.Log.Debugf("Adding static file: %s", webPath)
	body := string(content)
	replaceGlobal(&body)
	return body, nil
}
